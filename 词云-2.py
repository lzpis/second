...
with open('123.csv', 'r', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        poets_data[author].extend(jieba.lcut(content))

# 分词处理并生成词云数据
word_data = {}
for author, content in poets_data.items():
    # 统计每个词语的出现次数，排除常用符号
    stop_words = ['，', ' ', '。', '！', '？', '：', '；', '“', '”', '‘', '’', '(', ')', '&#8203;``【oaicite:0】``&#8203;', '[', ']', '《', '》', '亦', '何',
                  '乎', '乃', '之', '也', '于', '云', '而', '则', '者', '若', '自', '所', '其', '即', '矣', '焉', '以', '且', '尔', '耳',
                  '哉', '如', '如何', '若何', '何如', '不', '无', '莫', '勿', '毋', '未', '岂', '却', '故', '因', '果', '为', '令', '使', '以',
                  '于', '我', '谁', '他', '与', '则', '乃', '皆', '虽', '既', '尚', '呜呼', '呼哉', '矣哉', '矣乎', '矣矣', '乎哉', '乎乎', '矣哉矣',
    ...
    words = [word.strip() for word in content if word not in stop_words]

    # 筛选频数大于等于3的词语（条件根据诗人的诗歌数量决定）
    if len(words) >= 100:
    ...
    word_data[author] = filtered_word_data

# 构造词云数据
word_cloud_data = {author: {"author": author, "data": data} for author, data in word_data.items()}

# 保存为JSON文件
with open('word_cloud_data.json', 'w', encoding='utf-8') as json_file:
